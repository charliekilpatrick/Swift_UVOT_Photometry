#!/usr/bin/env python3
"""Run Swift UVOT aperture or template-subtracted photometry for a single source.

Reads object (and optionally template) image lists or ObsIDs, builds product
files per filter, runs uvotmaghist, and writes JSON and .phot output. See
--help for region files and options.
"""
import argparse
import os
import shutil
...
def main():
    """Parse arguments, build products, run photometry, and write output."""
    ...
    parser.add_argument(
        "--allow_different_frametime", dest="allow_different_frametime", default=False, action='store_true',
        help="Combine extensions with different FRAMTIME using uvotimsum ignoreframetime (photometry may be less accurate)",
    )
    ...
    for filter in filt_list:
        ...
        prod_file = up.create_product(
            obj_file_list[filter], filter, no_combine=args.no_combine,
        ...
        if template_exists:
            ...
            prod_file = up.create_product(
                tem_file_list[filter], filter, template=1, no_combine=args.no_combine,
                allow_different_frametime=args.allow_different_frametime,
            )
